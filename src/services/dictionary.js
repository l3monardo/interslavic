...
import { convertCases, deduplicate, filterLatin, filterNiqqud, getCaseTips, getCyrillic, getGender, getGlagolitic, getLatin, getNumeralType, getPartOfSpeech, getPronounType, isAnimate, isIndeclinable, isPlural, isSingular, latinToIpa, levenshteinDistance, normalize, removeBrackets, removeExclamationMark, searchStrategies, srGajevicaToVukovica, } from 'utils';
import { conjugationVerbFlat, declensionAdjectiveFlat, declensionNounFlat, declensionNounSimple, declensionNumeralFlat, declensionPronounFlat, } from '@interslavic/utils';
        ...
        switch (pos) {
            ...
            case 'noun': {
                ...
                if (details.includes('m./f.')) {
                    wordForms.push(...declensionNounFlat(wordElement, add, 'masculine', animate, plural, singular, indeclinable));
                    wordForms.push(...declensionNounFlat(wordElement, add, 'feminine', animate, plural, singular, indeclinable));
                }
                else {
                    wordForms.push(...declensionNounFlat(wordElement, add, gender, animate, plural, singular, indeclinable));
                }
                break;
            }
            case 'pronoun':
 ...
 */
function getAdditionFormFallback(item) {
    const add = Dictionary.getField(item, 'addition');
    if (add) {
        return add;
    }
...
class DictionaryClass {
    ...
    init(wordList, searchIndex, percentsOfChecked) {
        let startInitTime = 0;
        ...
        this.headerIndexes = new Map(this.header.map((item, i) => [this.header[i], i]));
        ...
        if (needIndex.length) {
            this.words.forEach((item) => {
                ...
                needIndex.forEach((lang) => {
                    ...
                    switch (lang) {
                        case ISV:
                            splittedField = deduplicate(this
                                .splittedMap[ISV_SRC]
                                .get(id)
    ...
    getIndex() {
        ...
        [
            ...
            searchIndex[lang] = Array.from(this.splittedMap[lang].keys()).map((key) => [
                key,
    ...
    translate(translateParams, showTime = true) {
        const { inputText, from, to, posFilter, flavorisationType, } = translateParams;
        ...
        const rawResults = this.getWordList()
            ...
            if (from === ISV || twoWaySearch) {
                ...
                filterResult = splittedField.some((chunk) => (searchTypes[searchType](chunk, hardEtymSearch ? inputWord : inputIsvPrepared)));
            }
            ...
            if ((from === ISV || twoWaySearch) &&
                ...
                filterResult = splittedField.some((chunk) => (searchTypes[searchType](this.applyIsvSearchLetters(chunk, flavorisationType), isvText)));
            }
            ...
            let dist = splittedField
                .reduce((acc, item) => {
                const lDist = levenshteinDistance(from === ISV ? inputIsvPrepared : inputLangPrepared, this.searchPrepare(from, item));
                if (lDist < acc) {
                    return lDist;
                }
                return acc;
            ...
            if (twoWaySearch) {
                ...
                const dist2 = splittedField
                    .reduce((acc, item) => {
                    const lDist = levenshteinDistance(from === ISV ? inputLangPrepared : inputIsvPrepared, this.searchPrepare(to, item));
                    if (lDist < acc) {
                        return lDist;
                    }
                    return acc;
    ...
    formatTranslate(results, from, to, flavorisationType, alphabets, caseQuestions) {
        return results.map((item) => {
            const isvRaw = this.getField(item, ISV);
            const remove = isvRaw.startsWith('!');
            const isv = removeBrackets(removeExclamationMark(isvRaw), '[', ']');
            const id = this.getField(item, 'id');
            ...
            if (alphabets?.cyrillic) {
                ...
                if (caseQuestions)
                    formattedItem.caseInfoCyr = getCyrillic(caseInfo, flavorisationType);
            }
            if (alphabets?.glagolitic) {
                ...
                if (caseQuestions)
                    formattedItem.caseInfoGla = getGlagolitic(caseInfo, flavorisationType);
            }
            return formattedItem;
        });
    }
    getPercentsOfTranslated() {
    ...
    changeIsvSearchLetters(letters) {
    ...
    setIsvSearchLetters(letters) {
        this.isvSearchLetters = letters;
    }
    setIsvSearchByWordForms(isvSearchByWordForms) {
        this.isvSearchByWordForms = isvSearchByWordForms;
    }
    inputPrepare(lang, text) {
    ...
    getSplittedField(from, item) {
        const key = this.getField(item, 'id');
        return this.splittedMap[from].get(key);
    }
    applyIsvSearchLetters(text, flavorisationType) {
        text = this.searchPrepare(ISV_SRC, text);
        isvReplacebleLetters
            .filter((replacement) => !this.isvSearchLetters.from.includes(replacement.from) ||
