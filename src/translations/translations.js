...
import { createTaggedTemplate, getCyrillic, getGlagolitic, getLatin, parseI18nString, } from 'utils';
let currentLang = EN;
...
function replaceParams(str, params) {
    return Object.keys(params).reduce((acc, paramKey) => (acc.replace(/[{}]/g, '').replace(new RegExp(`__${paramKey}__`, 'g'), params[paramKey])), str);
}
function tRaw(key) {
