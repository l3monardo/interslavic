...
export async function googleAuth(googleServiceAccountEmail, googlePrivateKey) {
    ...
    // eslint-disable-next-line
    // @ts-ignore
    const signingKey = await crypto.subtle.importKey('pkcs8', privateKeyBinary, {
        name: 'RSASSA-PKCS1-V1_5',
        hash: { name: 'SHA-256' },
    }, false, ['sign']);
    // Sign the googleJwtHeader and googleJwtClaimset
    const rawToken = await crypto.subtle.sign({ name: 'RSASSA-PKCS1-V1_5' }, signingKey, googleJwtCombined);
