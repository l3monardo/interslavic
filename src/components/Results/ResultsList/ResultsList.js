...
export const ResultsList = () => {
    ...
    return (_jsxs("div", { className: classNames('results-list', {
            short,
            empty: empty && !loading,
            idle,
        }), "data-testid": "results-list", style: {
            paddingLeft: scrollIsVisible ? scrollWidth : 0,
        }, ref: containerRef, children: [empty && !loading && (_jsx(ResultsEmpty, { showReset: posFilter !== '' })), results.map((item, index) => (_jsx(ResultsCard, { item: item, short: short, index: index, alphabets: alphabets, caseQuestions: caseQuestions }, item.id))), results.length > 0 && results.some((item) => !item.checked) && (_jsxs("div", { className: "results-list__message-for-users", children: [t('notVerifiedText').replace('part%', `${translatedPart}%`), ' ', _jsx(Link, { external: true, href: worksheetUrl, children: t('notVerifiedTableLinkText') })] }))] }));
};
